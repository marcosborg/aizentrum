...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.moloniItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.moloni-items.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('moloni_invoice') ? 'is-invalid' : '' }}" name="moloni_invoice_id" id="moloni_invoice_id" required>
                    @foreach($moloni_invoices as $id => $entry)
                        <option value="{{ $id }}" {{ old('moloni_invoice_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('suplier') ? 'is-invalid' : '' }}" type="text" name="suplier" id="suplier" value="{{ old('suplier', '') }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('synced') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="synced" id="synced" value="1" {{ old('synced', 0) == 1 ? 'checked' : '' }}>
